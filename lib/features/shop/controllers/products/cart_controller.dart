...
class CartController extends GetxController {
  ...
  void addItemToCart(ProductModel product) {
    ...
    if (outOfStock) {
      final message = variantId.isNotEmpty
  ...
  bool checkItemAlreadyExitsInCart(
    String productId, [
    String variationId = "",
  ]) {
  ...
  CartItemModel convertProductToCartItem(ProductModel product) {
    ...
    return CartItemModel(
      ...
      selectedVariation: Map<String, String>.from(
        _variationController.selectedAttributes,
      ),
    );
  }

  /// function used to save [cartItems] in localstorage
  ///
  /// [isAddItem] is a flag to check whether this action that call [save] is "Add item to cart" or not.
  ///
  /// [isAddItem = true] => show alter
  void save({bool isAddItem = true}) {
    try {
  ...
  void changeProductQuantityInCart({
    ...
    if (isPlus) {
      ...
      if (currentQty > 1) {
        cartItems[cartItemIndex] = currentItem.copyWith(
        ...
        final bool confirm = await TConfirmPopup.show(
          title: "This item in cart will be remove. Are you sure?",
        );
